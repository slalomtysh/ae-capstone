...
test('favorites -> home -> game detail -> home navigation', async ({ page }) => {
  ...
  await page.goto('/');
  await page.getByRole('link', { name: 'edit teams' }).click();

  await expect(page.getByRole('heading', { name: 'Team Selection' })).toBeVisible();
  await page.getByRole('button', { name: 'Buffalo Bills' }).click();
  await page.getByRole('button', { name: 'Save My Teams' }).click();

  await expect(page.getByRole('heading', { level: 2, name: "Today's Games" })).toBeVisible();
...
test('home shows partial data when one category fails', async ({ page }) => {
  await page.route('**/api/games/live**', async (route) => {
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({
        error: {
          ...
          message: 'Upstream failed and no eligible stale cache was available',
  ...
  await expect(page).toHaveURL('/');
  await expect(page.locator('.home-main .game-card').filter({ hasText: 'BUF' }).first()).toBeVisible();
  await expect(page.getByText('Upcoming games:')).toBeVisible();
  await expect(page.getByRole('button', { name: 'Retry Upcoming' })).toBeVisible();
});
